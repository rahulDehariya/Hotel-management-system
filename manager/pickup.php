...
$Department_Table_json=file_get_contents($HTTP_HOST."ajax.php?action=Department_tables&accNum=".$accNum);
$activeTables=json_decode($Department_Table_json,true);
    ...
    <nav class="navbar navbar-expand-lg  navbar-transparent">
        <div class="container-fluid">
          <div class="navbar-wrapper">
            ...
            <a style="color: #fff" class="navbar-brand" href="#pablo">Pick-Up</a>
...
function get_kitchen_show(){
  ...
  <div class="row"> -->
    ...
    <?php
     ...
     foreach ($activeTables as $DepartTable) {

     // print_r($DepartTable);
       $department=$DepartTable['department']['name'];
       $accNum =$DepartTable['department']['accNum'];
       

      $Table_name=$DepartTable['department']['active_tables'];
      if($Table_name!==""){
      foreach ($Table_name as $active_tbl1){
        $invoiceId =$active_tbl1['id'];
        $where=$active_tbl1['status'];


      
    ?>

      <div class="column kitchen_section kitchen_section" id="Table_itemsList_<?php echo $invoiceId;?>"style="">
      <!-- <div class="card demo-icons"> -->
      <section style="border: 2px solid #937e7e;padding: 5px;">

          <h2 style="border: none; margin: 0; text-align: center;font-size: 20px;color: #fff;"><?php echo $department; ?> / <?php echo $active_tbl1['info1']  ?> </h2>
          <ul style="padding:0;">
            <input type="hidden" id="Item_id" name="Item_id" value="<?php echo $active_tbl1['id']; ?>">
              ...
              // print_r($job_order);
                  ...
                  // echo "current ".date("Y-m-d H:i:s");
                      ...
                      // echo ($HTTP_HOST."ajax.php?action=GetItemDetails&id=".$invoiceId."&accNum=".$accNum."&status_where=".$where);//die;

                      $cartData_json=file_get_contents($HTTP_HOST."ajax.php?action=GetItemDetails&id=".$invoiceId."&accNum=".$accNum."&status_where=".$where);

                      $cartData=json_decode($cartData_json,true);
                      // print_r($cartData_json);
                      // $order_details=$cartData_json['order_details'];
                 ...
                 ?>
                  ...
                  <li id="refresh_items" onclick="GetIvoiceDetail(<?php echo $active_tbl1['id']; ?>)" data-toggle="modal" data-target="#myModal" style="cursor:pointer;width: 98%; font-size: 14px;color: #fff;margin: 1%;">
                  <!-- <i class="nc-icon nc-alert-circle-i"></i> -->
                  <p style="width: 100%;    float: left;">
                   <span class="img_offer badge badge-warning" ></span></p>
                    <div id="list_items" style="text-align: left ;margin-left: 2px;border-bottom: 1px solid #f7f7f7; color: #2c7114;">
                  ...
                  <!-- <p style="font-size: 19px;">Job-<?php echo $data['jobNumber'];  ?></p> -->
                    ...
                    <?php
                      ...
                      foreach ($cartData['order_details']['items_full'] as  $item_name) {
                        // print_r($item_name);
                        ?>

                          <div id="invoiceItemList_<?php echo $item_name['id'];  ?>" style="text-align: left ;margin-left: 2px;border-bottom: 1px solid #f7f7f7; color: #f7f4f4;;"><?php echo $item_name['info1'] ?>

                         <span style="text-align: right;margin-right:2px;float: right;"><?php echo $item_name['qty']  ?></span></div>
  ...
  $p++;
            ...
            <div class="modal-header">
              ...
              <button type="button" class="close" data-dismiss="modal">&times;</button>
                  ...
                  <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" onclick="set_all_prepared()">All Prepared</button>
                    <button type="button" class="btn btn-secondary" onclick="set_all_pickedup()">All Picked-Up</button>
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                  </div>
            </div>
        </div>
      ...
      <script type="text/javascript">
        ...
        function set_all_prepared()
        {
          invoiceId = $("#hidden_invoice_id").val();
          $.ajax({
                type: 'POST',
                url: 'ajax.php?action=set_all_prepared&invoiceId='+invoiceId,
                data: {},
                success: function (response)
                {
                  //alert(response);
                  // $("#item_prepared").remove();
                  swal("Items Prepared successfully","success");
                  $("#item_prepared").hide();
                }
            });
        }
        function set_all_pickedup()
        {
          invoiceId = $("#hidden_invoice_id").val();
          $.ajax({
                type: 'POST',
                url: 'ajax.php?action=set_all_pickedup&invoiceId='+invoiceId,
                data: {},
                success: function (response)
                {
                  // alert("Action Performed successfully");
                  
                  swal("Items Picked-Up successfully","success");
                  $("#Cart_items_list").hide();


                  // $("#Table_itemsList").load();
                  // $('#myModal').modal('toggle');
                  $("#Table_itemsList_"+invoiceId).remove();
                }
            });
        }
          setTimeout(function(){
      ...
      // alert('ajax.php?action=getjobItems&Id='+jobId+"&location="+location+"&invoiceId="+invoiceId);
              ...
              // $("#job_items_list").html("");
                ...
                for(var p=0;p<obj.length;p++)
                {
                 ...
                 // job_items +='<tr><td>'+K+'</td><td>'+obj[p]['itemCode']+'</td><td>'+obj[p]['info1']+'</td><td>'+obj[p]['qty']+'</td><td>'+obj[p]['itemName']+'</td><td>';
                  ...
                  job_items +='<button id="item_pickedup" onclick=Get_item_pickedup('+obj[p]['id']+') class="btn btn-primary btn-round  ">Picked-Up</button></td></tr>'
                }
        ...
        function Get_item_prepared(id)
        {
          ...
          $.ajax({
            ...
            {
              // alert(data);
              swal("Item Prepared successfully","success");
              $("#item_prepared").hide();
        ...
        // GetIvoiceDetail();
         function GetIvoiceDetail(id){
          // alert(id);
          //var invoiceId=$("#Item_id").val();
          var invoiceId=id;

         $.ajax({
              type: 'POST',
              url: 'ajax.php?action=GetItemDetails&id='+invoiceId,
              data: {},
              success: function (response) {
                  // alert(response);
                ...
                {
                  K=p+1;
                  Item_listData+='<tr id="item_row_'+items[p]['id']+'"><input type="hidden" id="Item_Code" value="'+items[p]['invoiceId']+'"><td>'+K+'</td><td>'+data['info1']+'</td><td>'+items[p]['itemCode']+'</td><td>'+items[p]['info1']+'</td><td>'+items[p]['qty']+'</td><td>';

                  if(items[p]['status']!=3)
                  {
                  Item_listData+= '<button onclick=Get_item_prepared('+items[p]['id']+')  id="item_prepared" class="btn btn-primary btn-round" style="margin-left: 100px;">Prepared</button> ';
                   }
                   Item_listData+='<button id="item_pickedup" onclick=Get_item_pickedup('+items[p]['id']+') class="btn btn-primary btn-round">PickedUp</button></td></tr>'

                }
                $("#Cart_items_list").html(Item_listData);

                  if(Item_listData == "")
                 {
                   Item_listData+='<tr id="item"><td class="cart_items_empty" colspan=4>No item added</td></tr>';
                 }
                 $("#Cart_items_list").html(Item_listData);
